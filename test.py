...
def identify_and_mark_shapes(image_path):
    ...
    for i in range(min(2, len(sorted_contours))):
        ...
        cv2.circle(original_img, (cx, cy), 5, (0, 0, 0), -1)

        # Mark the largest shapes
        cv2.putText(original_img, f"Largest-{i + 1}", (cx - 30, cy - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
...
# Example usage
identify_and_mark_shapes("final.png")
